...
use DB;
use Session;
use Modules\Hrms\Entities\HrmsTraining;
use Modules\Hrms\Entities\HrmsPosting;
use Modules\Hrms\Entities\HrmsDisciplinaryAction;
use Modules\Hrms\Entities\HrmsAward;
use Modules\Hrms\Entities\HrmsPromotion;
use Modules\Hrms\Entities\HrmsPublication;
...
class EmployeesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $attribute = [
            'employee_bpc_id'              => 'Employee BPC ID',
        ...
        $employeeData = [
            'employee_bpc_id' => $request['employee_bpc_id'],
    ...
    {        
		...
		if($request->method() === 'POST' ){
            dd($request->method());
            $employee = HrmsEmployee::where('employee_id', $id)->first();
            $postings = HrmsPosting::where('employee_id', $employee->employee_bpc_id)->get();
            $tarinings = HrmsTraining::where('employee_id', $employee->employee_bpc_id)->get();
            $publications = HrmsPublication::where('employee_id', $employee->employee_bpc_id)->get();
            $promotions = HrmsPromotion::where('employee_id', $employee->employee_bpc_id)->get();
            $awards = HrmsAward::where('employee_id', $employee->employee_bpc_id)->get();
            $actions = HrmsDisciplinaryAction::where('employee_id', $employee->employee_bpc_id)->get();
            array_splice($postings, $tarinings, $publications, $promotions, $awards, $actions);
			$employee->delete();
