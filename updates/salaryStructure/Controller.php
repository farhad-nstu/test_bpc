...
class SalaryStructureController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $addData = $request->only('rule_id','code','condition','percentage','amount');
        $deducData = $request->only('deductionRule_id','deductionCode','deductionCondition','deductionPercentage','deductionAmount');
    ...
    public function fetch_rule_details(Request $request)
    {
        ...
        if($rule->is_fixed == 0) {
            ...
            $condition = "(BS * Percentage) / 100";
            $percentage = $rule->rules_amount;
        }

        return response()->json(['code' => $rule->rules_code, 'amount' => floor($amount), 'condition' => $condition, 'percentage' => $percentage]);
        // return $rule;
    }
    ...
    public function fetch_deduc_rule_details(Request $request)
    {
        ...
        if($deducRule->is_fixed == 0) {
            ...
            $condition = "(BS * Percentage) / 100";
            $percentage = $deducRule->rules_amount;
        }

        return response()->json(['code' => $deducRule->rules_code, 'amount' => floor($amount), 'condition' => $condition, 'percentage' => $percentage]);
        // return $deducRule;
    }
