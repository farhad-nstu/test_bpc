...
class SalaryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ( empty($id) ){
            ...
            // $recentSalary = $this->model::where('employee_id', $request['employee_id'])->where('salary_date', substr($request['salary_date'], 0, 7))->first();
            ...
            foreach($loanManages as $loanManage){
                    ...
                    if($loanTitle == "House Building Loan" && $loanManage->current_or_one_year == "one_year_later") {
                        ...
                        if($request['salary_date'] >= $nextMonth) {
                            if($loanManage->rest_amount > 0) {
                                ...
                                $loanInterestData = [
                                    ...
                                    'date' => substr($request['salary_date'], 0, 7),
