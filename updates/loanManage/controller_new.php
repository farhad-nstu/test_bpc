...
class LoanManageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $loan = HrmsLoan::where('loan_id', $request['loan_id'])->first();

        if($request['amount'] > $loan->amount) {
            return redirect()->back()->withErrors('Amount can not exceed loan amount')->withInput();
        }

        if($request['installment'] > $loan->installment) {
            return redirect()->back()->withErrors('Installment can not exceed loan installment')->withInput();
        }
        ...
        $loanManageData = [
            ...
            'rest_amount' => $request['amount'] - $request['paid_amount'],
            'current_or_one_year' => $request['current_or_one_year'],
        ];
    ...
    public function fetch_loan_details(Request $request)
    {
        ...
        return response()->json(['max_amount' => $loan->amount, 'max_installment' => $loan->installment, 'interest' => $loan->interest]);
        // return $loan;
    }
    ...
    public function fetch_paid_amount(Request $request)
    {
        ...
        // $payableAmount = $request->amount + $amountInterest;
        // $monthlyPayableAmount = $payableAmount / $request->installment;
        $paidAmount = $request->monthly_payment * $request->paid_installment;
        return $paidAmount;
    }
}
