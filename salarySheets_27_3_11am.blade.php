    ...
    <div class="card-body" id="">
      ...
      <div class="col-md-10">
        ...
        <form action="{{url($bUrl)}}" method="get" class="form-inline">
          ...
          <div class="form-row">
            <div class="col">
              <input type="text" name="filter" value="{{ $filter ?? '' }}" placeholder="Filter by employee id ..."
          ...
          <table class="table table-bordered">
              ...
              <tbody>
                ...
                @foreach($allData as $query)
                  ...
                  @php
                    ...
                    foreach($employeeLoans as $employeeLoan){
                      array_push($loansId, $employeeLoan->loan_id);
                    }
                ...
                <tr>
                  ...
                  <td class="text-center">{{ $data->employee_bpc_id }}</td>
                  ...
                  <td class="text-center">{{ ceil($grossTotal) }}</td>
                  <td class="text-center">{{ ceil($gpf) }}</td>
                  ...
                  @foreach($loans as $loan)
                    @if(in_array($loan->loan_id, $loansId))
                      @php 
                        $employeeLoan = $employeeLoans->where('loan_id', $loan->loan_id)->first();
                        $amountInterest = ($employeeLoan->amount*$employeeLoan->interest) / 100;
                        $payableAmount = $employeeLoan->amount + $amountInterest;
                        $monthlyPayableAmount = $payableAmount / $employeeLoan->installment;
                        $loanAmount = $loanAmount + $monthlyPayableAmount;
                        echo "<td class='text-center'>".ceil($monthlyPayableAmount)."</td>";
                      @endphp 
                    @else 
                      <td class="text-center">0</td>
                    @endif
                  ...
                  <td class="text-center">{{ ceil($totalDeduction + $loanAmount) }}</td>
                  <td class="text-center">{{ ceil($totalSalary - $loanAmount) }}</td>
